...
public final class ExcelUtils {
     ...
     * @return
     */
    public static void setAlignRight(CellStyle cellStyle) {
    ...
    public static CellStyle createHeaderStyle(Workbook workbook) {
        ...
        ExcelUtils.setTitileFont(titleStyle, titleFont, (short) 11, "宋体");
        return titleStyle;
    }
    ...
    public static CellStyle createCommonCellStyle(Workbook workbook) {
        ...
        Font titleFont = workbook.createFont();
        ExcelUtils.setFont(cellStyle, titleFont, (short) 11, "宋体");
        return cellStyle;
    }
     ...
     * 导出Excel，表头行
     */
    public static void writeHeaderRow(Sheet sheet, Row titleRow, FieldInfo[] fieldInfoArray, CellStyle headerStyle) {
        int i = 0;
        int columnWidth = 0;
        Cell cell = null;
     ...
     * @return
     */
    private static Object getObjectValue(Cell cell, FieldInfo fieldInfo) throws ParseException {
        if (cell == null) {
            return null;
        }
        Object value = null;
        ...
        switch (cell.getCellType()) {
            case XSSFCell.CELL_TYPE_STRING:
                if (convertor != null) {
                    value = convertor.inport(StringUtils.trim(cell.getStringCellValue()));
                } else {
                ...
                (fieldInfo.getFieldType() == Integer.class) {
                    value = value == null ? (Integer) value : Integer.valueOf((String) value);
                } else if (fieldInfo.getFieldType() == Long.class) {
                    value = value == null ? (Long) value : Long.valueOf((String) value);
                } else if (fieldInfo.getFieldType() == Double.class) {
                    value = value == null ? (Double) value : Double.valueOf((String) value);
                } else if (fieldInfo.getFieldType() == BigDecimal.class) {
                    value = value == null ? (BigDecimal) value : new BigDecimal((String) value);
                } else if (fieldInfo.getFieldType() == Float.class) {
                    value = value == null ? (Float) value : Float.valueOf((String) value);
                } else if (fieldInfo.getFieldType() == Boolean.class) {
                    value = value == null ? (Boolean) value : Boolean.valueOf((String) value);
                } else if (fieldInfo.getFieldType() == Date.class) {
                    if (value != null) {
                        value = fieldInfo.getDataFormater().parse((String) value);
                    } else {
                        value = (Date) value;
                    }
                } else {
            ...
            case XSSFCell.CELL_TYPE_NUMERIC:
                if (HSSFDateUtil.isCellDateFormatted(cell)) {
                    if (convertor != null) {
                        ...
                        if (fieldInfo.getFieldType() == String.class) {
                            value = value == null ? (String) value : fieldInfo.getDataFormater().format((Date) value);
                        }
                    }
                } else {
                    ...
                    if (convertor != null) {
                        ...
                        if (fieldInfo.getFieldType() == Integer.class) {
                            value = value == null ? (Integer) value : ((Double) value).intValue();
                        } else if (fieldInfo.getFieldType() == Long.class) {
                            value = value == null ? (Long) value : ((Double) value).longValue();
                        } else if (fieldInfo.getFieldType() == BigDecimal.class) {
                            if (value != null) {
                            ...
                            value = value == null ? (Float) value : ((Double) value).floatValue();
                        } else if (fieldInfo.getFieldType() == String.class) {
                            if (fieldInfo.getNumberFormater() != null && value != null) {
                                value = fieldInfo.getNumberFormater().format(value);
                            } else {
                                value = (String) value;
                            }
                        } else {
            ...
            case XSSFCell.CELL_TYPE_FORMULA:
                if (convertor != null) {
                    value = convertor.inport(StringUtils.trim(cell.getCellFormula()));
                } else {
    ...
    public static <T> void writeDateToSheet(Workbook book, Sheet sheet, List<T> listData, int startRow,
        ...
        try {
            ...
            Row row = null;
            ExcelUtils.createCellStyle(fieldInfoArr, book);
            for (T t : listData) {
                row = ExcelUtils.getOrCreateRow(sheet, startRow);
                writeOneData(row, t, fieldInfoArr);
                startRow++;
            }
        } catch (Exception e) {
    ...
    public static void createCellStyle(FieldInfo[] fieldInfoArr, Workbook book) {
        DataFormat dataFormat;
        CellStyle cellStyle = null;
        for (FieldInfo info : fieldInfoArr) {
            cellStyle = ExcelUtils.createCommonCellStyle(book);
            if (info.getAlignment() < 0) {
            ...
            if (StringUtils.isNotBlank(info.getNumberFromat())) {
                cellStyle.setDataFormat(book.createDataFormat().getFormat(info.getNumberFromat()));
            } else if (StringUtils.isNotBlank(info.getDateFormat())) {
                cellStyle.setDataFormat(book.createDataFormat().getFormat(info.getDateFormat()));
            }
     ...
     * @throws Exception
     */
    private static <T> void writeOneData(Row row, T t, FieldInfo[] fieldInfoArr) throws Exception {
        ...
        try {
            for (int i = 0; i < len; i++) {
                ...
                writeDataToCell(cell, value, fieldInfo);
            }
        } catch (Exception e) {
     ...
     * @param fileInfo
     */
    public static void writeDataToCell(Cell cell, Object value, FieldInfo fileInfo) {
        if (cell == null) {
            return;
        }
        CellStyle cellStyle = fileInfo.getCellStyle();
        if (fileInfo.getConvertor() != null) {
            value = fileInfo.getConvertor().export(value);
        }
        if (value == null) {
            cell.setCellStyle(fileInfo.getCellStyle());
            return;
        }
     ...
     */
    public static void setBlackColorStyle(Workbook workBook, Sheet sheet, int recordCount, int cellSize) {
        if (recordCount == 0) {
            return;
        }
     ...
     */
    public static void setRedColorStyle(Workbook workBook, Sheet sheet, Map<Integer, Set<Integer>> invalidCellIndex) {
